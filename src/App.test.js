...
test("app fetching the show api and renders episodes to the dom", async () => {
    //set the mock
    mockFetchShow.mockResolvedValueOnce(show);

    //render app - should show image and p and h1
    const {findByText, getByTestId, getAllByTestId} = render(<App />);


//check for intermidiate state while the api call is running
await findByText(/fetching data/i);

//this pssses after the data response comes through
const showInfo =  getByTestId(/show name/i);
 ...
 await waitFor(() => {
//   const episode = screen.getByText(/Chapter One: The Vanishing of Will Byers/i);
